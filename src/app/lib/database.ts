...
export const dbOperations = {
  ...
  async createMissionMetadata(metadataData: {
    ...
    if (!supabase) {
      console.error('❌ Supabase client not initialized for mission_metadata creation');
      return null;
    }

    console.log('🔒 Creating mission metadata for session:', metadataData.missionSessionId);
    console.log('🔒 Metadata contains:', {
      phasesCount: metadataData.detailedPhases.length,
      successConditionsCount: metadataData.successConditions.length,
      failureConditionsCount: metadataData.failureConditions.length,
      outcomesCount: metadataData.possibleOutcomes.length
    });
    ...
    try {
      ...
      console.log('✅ Mission metadata created successfully with ID:', data.id);
      return data.id;
    } catch (err) {
      console.error('❌ Exception in createMissionMetadata:', err);
      return null;
    }
  },
